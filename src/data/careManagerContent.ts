export const careManagerContent = {
  hero: {
    badge: "For Experienced Care Professionals",
    headline: "You're Already a Care Professional.",
    headlineHighlight: "Now Earn What You Deserve.",
    subheadline: "Become a Telth Care Manager and turn your care experience into your own independent practice",
    description: "Working at a care home, nursing facility, or already doing home visits? Your years of experience deserve better compensation and the freedom to build your own client base. Start with no overhead for basic care. Just your expertise and our platform.",
    primaryCTA: "Become a Care Manager",
    secondaryCTA: "Calculate Your Earnings",
  },
  earnings: {
    perVisit: "₹300 - ₹500",
    perVisitDesc: "per home visit",
    subscription: "₹1,000 - ₹2,000",
    subscriptionDesc: "/month per ongoing client",
    referral: "₹500 - ₹1,000",
    referralDesc: "per new patient referral",
  },
  finalCTA: {
    headline: "You've Cared for Others Your Whole Career. Now Care for Your Future.",
    button: "Become a Care Manager Today",
    socialProof: "Join Telth Care Managers from nursing homes, palliative care facilities, and home care services across India who are building independence",
  },
};
